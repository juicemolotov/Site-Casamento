...
<head>
  ...
  <title>Gabriele & Matheus</title>
  <link href="https://fonts.googleapis.com/css2?family=Great+Vibes&family=Crimson+Text&display=swap" rel="stylesheet">
  <style>
    body {
      margin: 0;
    ...
    header {
      ...
      background-image: url('https://i.imgur.com/FtRUZ0K.png'); /* topo floral */
...
<body>
  ...
  <script>
    const countdown = () => {
      const end = new Date('2025-11-29T16:30:00');
      const now = new Date();
      const diff = end - now;
      if (diff <= 0) {
        document.getElementById("countdown").innerText = "É hoje! 💍";
        return;
      }
      ...
      document.getElementById("countdown").innerText = `${days} dias, ${hours}h ${minutes}m ${seconds}s`;
    };
